...
def _test_setup_common_training_handlers(
    dirname, device, rank=0, local_rank=0, distributed=False
):
    ...
    assert optimizer.param_groups[0]["lr"] <= lr * gamma ** (
        ...
        lr * gamma ** (num_iters * num_epochs / step_size),
    )
...
def test_asserts_setup_common_training_handlers():
    ...
    with pytest.raises(
        ValueError,
        match=r"If to_save argument is provided then output_path argument should be "
        r"also defined",
    ):
    ...
    with pytest.warns(
        UserWarning,
        match=r"Argument train_sampler distributed sampler used to call "
        r"`set_epoch` method on epoch",
    ):
        train_sampler = MagicMock()
        setup_common_training_handlers(
            trainer, train_sampler=train_sampler, with_gpu_stats=False
        )
...
def test_save_best_model_by_val_score(dirname, capsys):
    ...
    save_best_model_by_val_score(
        dirname, evaluator, model, metric_name="acc", n_saved=2, trainer=trainer
    )
    ...
    assert set(os.listdir(dirname)) == set(
        ["best_model_8_val_acc=0.61.pth", "best_model_9_val_acc=0.7.pth"]
    )
...
def test_add_early_stopping_by_val_score():
    ...
    add_early_stopping_by_val_score(
        patience=3, evaluator=evaluator, trainer=trainer, metric_name="acc"
    )
    ...
    def _test(with_eval, with_optim):
        ...
        if with_optim:
            ...
            for cls in [
                ...
                assert any([isinstance(h[0], cls) for h in handlers]), "{}".format(
                    handlers
                )
...
def test_distrib_gpu(dirname, distributed_context_single_node_nccl):
    ...
    _test_setup_common_training_handlers(
        dirname, device, rank=local_rank, local_rank=local_rank, distributed=True
    )
...
def test_multinode_distrib_gpu(dirname, distributed_context_multi_node_nccl):
    ...
    device = "cuda:{}".format(local_rank)
    _test_setup_common_training_handlers(
        dirname, device, rank=rank, local_rank=local_rank, distributed=True
    )
    test_add_early_stopping_by_val_score()
