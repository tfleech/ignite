...
def test_no_pynvml_package(no_site_packages):
    ...
    with pytest.raises(
        RuntimeError, match="This contrib module requires pynvml to be installed."
    ):
        GpuInfo()


@pytest.mark.skipif(
    sys.version[0] == "2" or torch.cuda.is_available(),
    reason="No pynvml for python 2.7",
)
def test_no_gpu():

    with pytest.raises(
        RuntimeError, match="This contrib module requires available GPU"
    ):
        GpuInfo()
...
def _test_gpu_info(device="cpu"):
    ...
    assert (
        mem_report["used"]
        > t.shape[0] * t.shape[1] * t.shape[2] * t.shape[3] / 1024.0 / 1024.0
    )
    ...
    assert (
        int(mem_report["used"] * 100.0 / mem_report["total"])
        == engine.state.metrics["gpu:0 mem(%)"]
    )
...
@pytest.mark.skipif(
    sys.version[0] == "2" or not (torch.cuda.is_available()),
    ...
    with patch.dict(
        ...
        def query(*args, **kwargs):
            return {
                "gpu": [
                    {
                        "fb_memory_usage": {"used": 100.0, "total": 11000.0},
                        "utilization": {"gpu_util": 50.0},
                    }
                ]
            }
...
def test_gpu_info_mock(mock_pynvml_module, mock_gpu_is_available):
    ...
    _test_with_custom_query(
        resp={}, warn_msg=r"No GPU information available", check_compute=True
    )
