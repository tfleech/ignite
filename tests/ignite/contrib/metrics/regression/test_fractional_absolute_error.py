...
def test_compute():
    ...
    np_sum += (
        2 * np.abs((b - ground_truth)) / (np.abs(b) + np.abs(ground_truth))
    ).sum()
    np_len += len(b)
    np_ans = np_sum / np_len
    assert m.compute() == pytest.approx(np_ans)
    ...
    np_sum += (
        2 * np.abs((c - ground_truth)) / (np.abs(c) + np.abs(ground_truth))
    ).sum()
    np_len += len(c)
    np_ans = np_sum / np_len
    assert m.compute() == pytest.approx(np_ans)
    ...
    np_sum += (
        2 * np.abs((d - ground_truth)) / (np.abs(d) + np.abs(ground_truth))
    ).sum()
    np_len += len(d)
    np_ans = np_sum / np_len
    assert m.compute() == pytest.approx(np_ans)
