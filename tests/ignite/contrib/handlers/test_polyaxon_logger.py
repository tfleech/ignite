...
def test_output_handler_with_wrong_logger_type():
    ...
    with pytest.raises(
        RuntimeError, match="Handler 'OutputHandler' works only with PolyaxonLogger"
    ):
...
def test_output_handler_output_transform():
    ...
    mock_logger.log_metrics.assert_called_once_with(
        step=123, **{"another_tag/loss": 12345}
    )
...
def test_output_handler_metric_names():
    ...
    mock_logger.log_metrics.assert_called_once_with(
        step=5, **{"tag/a": 12.23, "tag/b": 23.45, "tag/c": 10.0}
    )
    ...
    mock_logger.log_metrics.assert_has_calls(
        [
            call(
                step=5,
                **{"tag/a/0": 0.0, "tag/a/1": 1.0, "tag/a/2": 2.0, "tag/a/3": 3.0}
            ),
        ],
        any_order=True,
    )
    ...
    assert mock_logger.log_metrics.call_count == 1
    mock_logger.log_metrics.assert_has_calls(
        [call(step=7, **{"tag/a": 55.56})], any_order=True
    )
...
def test_output_handler_both():
    ...
    assert mock_logger.log_metrics.call_count == 1
    mock_logger.log_metrics.assert_called_once_with(
        step=5, **{"tag/a": 12.23, "tag/b": 23.45, "tag/loss": 12345}
    )
...
def test_optimizer_params_handler_wrong_setup():
    ...
    with pytest.raises(
        RuntimeError,
        match="Handler 'OptimizerParamsHandler' works only with PolyaxonLogger",
    ):
...
def test_optimizer_params():
    ...
    wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
    mock_logger.log_metrics.assert_called_once_with(
        **{"generator/lr/group_0": 0.01, "step": 123}
    )
...
def test_integration_as_context_manager():
    ...
    with PolyaxonLogger() as plx_logger:
        ...
        def dummy_handler(engine, logger, event_name):
            global_step = engine.state.get_event_attrib_value(event_name)
            logger.log_metrics(
                step=global_step, **{"{}".format("test_value"): global_step}
            )
...
def test_no_polyaxon_client(no_site_packages):
    ...
    with pytest.raises(
        RuntimeError,
        match=r"This contrib module requires polyaxon-client to be installed",
    ):
        PolyaxonLogger()
