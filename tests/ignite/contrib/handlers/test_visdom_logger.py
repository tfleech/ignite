...
def visdom_server():
    ...
    port = 8098
    p = subprocess.Popen(
        "visdom --hostname {} -port {}".format(hostname, port), shell=True
    )
    time.sleep(5)
    yield (hostname, port)
    p.terminate()
...
def test_optimizer_params():
    ...
    wrapper = OptimizerParamsHandler(
        optimizer=optimizer, param_name="lr", tag="generator"
    )
    mock_logger = MagicMock(spec=VisdomLogger)
    mock_logger.vis = MagicMock()
    mock_logger.executor = _DummyExecutor()
...
def test_output_handler_with_wrong_logger_type():
    ...
    mock_engine = MagicMock()
    with pytest.raises(
        RuntimeError, match="Handler 'OutputHandler' works only with VisdomLogger"
    ):
        wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
...
def test_output_handler_metric_names(dirname):
    ...
    assert len(wrapper.windows) == 4 and all(
        ["tag/a/{}".format(i) in wrapper.windows for i in range(4)]
    )
...
def test_output_handler_both(dirname):
    ...
    wrapper = OutputHandler(
        "tag", metric_names=["a", "b"], output_transform=lambda x: {"loss": x}
    )
    mock_logger = MagicMock(spec=VisdomLogger)
    mock_logger.vis = MagicMock()
    mock_logger.executor = _DummyExecutor()
...
def test_weights_scalar_handler_wrong_setup():
    ...
    with pytest.raises(
        TypeError, match="Argument model should be of type torch.nn.Module"
    ):
    ...
    with pytest.raises(
        ValueError, match="Output of the reduction function should be a scalar"
    ):
        ...
        mock_logger.vis.line.assert_has_calls(
            [
                call(
                    ...
                    win=None,
                    update=None,
                    opts=wrapper.windows[tag_prefix + "weights_norm/fc1/weight"][
                        "opts"
                    ],
                ...
                call(
                    ...
                    win=None,
                    update=None,
                    opts=wrapper.windows[tag_prefix + "weights_norm/fc2/weight"][
                        "opts"
                    ],
...
def test_grads_scalar_handler_wrong_setup():
    ...
    mock_engine = MagicMock()
    with pytest.raises(
        RuntimeError, match="Handler 'GradsScalarHandler' works only with VisdomLogger"
    ):
        wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
...
def test_logger_init_hostname_port(visdom_server):
    ...
    vd_logger = VisdomLogger(
        server=visdom_server[0], port=visdom_server[1], num_workers=0
    )
...
def test_integration_no_executor(visdom_server):
    ...
    output_handler = OutputHandler(
        tag="training", output_transform=lambda x: {"loss": x}
    )
    vd_logger.attach(
        trainer, log_handler=output_handler, event_name=Events.ITERATION_COMPLETED
    )
...
def test_integration_with_executor(visdom_server):
    vd_logger = VisdomLogger(
        server=visdom_server[0], port=visdom_server[1], num_workers=1
    )
...
def test_no_visdom(no_site_packages):
    ...
    with pytest.raises(
        RuntimeError, match=r"This contrib module requires visdom package"
    ):
        VisdomLogger()
