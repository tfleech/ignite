...
def test_base_output_handler_wrong_setup():
    ...
    with pytest.raises(
        TypeError, match="metric_names should be either a list or equal 'all'"
    ):
    ...
    with pytest.raises(
        ValueError, match="Either metric_names or output_transform should be defined"
    ):
        DummyOutputHandler("tag", None, None)

    with pytest.raises(
        TypeError, match="Argument another_engine should be of type Engine"
    ):
...
def test_base_output_handler_with_another_engine():
    ...
    with pytest.warns(DeprecationWarning, match="Use of another_engine is deprecated"):
        handler = DummyOutputHandler(
            "tag", metric_names=["a", "b"], output_transform=None, another_engine=engine
        )
...
def test_base_output_handler_setup_output_metrics():
    ...
    # Only output as "loss"
    handler = DummyOutputHandler(
        "tag", metric_names=None, output_transform=lambda x: {"loss": x}
    )
    metrics = handler._setup_output_metrics(engine=engine)
