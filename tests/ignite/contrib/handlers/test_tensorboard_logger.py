...
def test_optimizer_params_handler_wrong_setup():
    ...
    with pytest.raises(
        RuntimeError,
        match="Handler 'OptimizerParamsHandler' works only with TensorboardLogger",
    ):
...
def test_optimizer_params():
    ...
    wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
    mock_logger.writer.add_scalar.assert_called_once_with(
...
def test_output_handler_with_wrong_logger_type():
    ...
    mock_engine = MagicMock()
    with pytest.raises(
        RuntimeError, match="Handler 'OutputHandler' works only with TensorboardLogger"
    ):
        wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
...
def test_output_handler_both(dirname):
    ...
    mock_logger.writer.add_scalar.assert_has_calls(
        [call("tag/a", 12.23, 5), call("tag/b", 23.45, 5), call("tag/loss", 12345, 5)],
        any_order=True,
    )
...
def test_weights_scalar_handler_wrong_setup():
    ...
    with pytest.raises(
        RuntimeError,
        match="Handler 'WeightsScalarHandler' works only with TensorboardLogger",
    ):
        wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
...
def test_weights_hist_handler_wrong_setup():
    ...
    with pytest.raises(
        RuntimeError,
        match="Handler 'WeightsHistHandler' works only with TensorboardLogger",
    ):
        wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
...
def test_grads_scalar_handler_wrong_setup():
    ...
    with pytest.raises(
        RuntimeError,
        match="Handler 'GradsScalarHandler' works only with TensorboardLogger",
    ):
        wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
...
def test_grads_hist_handler_wrong_setup():
    ...
    with pytest.raises(
        RuntimeError,
        match="Handler 'GradsHistHandler' works only with TensorboardLogger",
    ):
        wrapper(mock_engine, mock_logger, Events.ITERATION_STARTED)
...
def test_integration(dirname):
    ...
    tb_logger.attach(
        trainer, log_handler=dummy_handler, event_name=Events.EPOCH_COMPLETED
    )
...
def test_no_tensorboardX_nor_torch_utils_tensorboard():
    with patch.dict(
        "sys.modules", {"tensorboardX": None, "torch.utils.tensorboard": None}
    ):
        with pytest.raises(
            RuntimeError,
            match=r"This contrib module requires either tensorboardX or torch",
        ):
