...
def test_binary_wrong_inputs():
    ...
    with pytest.raises(ValueError):
        # incompatible shapes
        re.update(
            (torch.randint(0, 2, size=(10,)), torch.randint(0, 2, size=(10, 5)).long())
        )
...
def test_binary_input_N():
    ...
    def _test(average):
        ...
        assert recall_score(np_y, np_y_pred, average="binary") == pytest.approx(
            re_compute
        )
...
def test_multiclass_wrong_inputs():
    ...
    with pytest.raises(ValueError):
        # incompatible shapes between two updates
        re.update(
            (torch.rand(10, 5, 12, 14), torch.randint(0, 5, size=(10, 12, 14)).long())
        )
        re.update(
            (torch.rand(10, 6, 12, 14), torch.randint(0, 5, size=(10, 12, 14)).long())
        )
...
def test_multiclass_input_NL():
    ...
    def _test(average):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=UndefinedMetricWarning)
            assert recall_score(
                np_y, np_y_pred, average=sk_average_parameter
            ) == pytest.approx(re_compute)
...
def test_multilabel_wrong_inputs():
    ...
    with pytest.raises(ValueError):
        ...
        re.update(
            (
                torch.randint(0, 2, size=(20, 5)),
                torch.randint(0, 2, size=(20, 5)).long(),
            )
        )
        re.update(
            (
                torch.randint(0, 2, size=(20, 6)),
...
def test_multilabel_input_NC():
    def _test(average):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=UndefinedMetricWarning)
            assert recall_score(np_y, np_y_pred, average="samples") == pytest.approx(
                re_compute
            )
...
def _test_distrib_itegration_multilabel(device):
    ...
    def _test(average, n_epochs):
        ...
        y_true = torch.randint(
            0, 2, size=(offset * dist.get_world_size(), n_classes, 6, 8)
        ).to(device)
