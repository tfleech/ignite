...
def test_wrong_input_args():
    ...
    with pytest.raises(
        ValueError, match=r"Argument output_transform should be None if src is a Metric"
    ):
        RunningAverage(Accuracy(), output_transform=lambda x: x[0])

    with pytest.raises(
        ValueError,
        match=r"Argument output_transform should not be None if src corresponds",
    ):
        RunningAverage()

    with pytest.raises(
        ValueError, match=r"Argument device should be None if src is a Metric"
    ):
...
def test_epoch_unbound():
    ...
    y_true_batch_values = iter(
        np.random.randint(0, n_classes, size=(n_epochs * n_iters, batch_size))
    )
    y_pred_batch_values = iter(
        np.random.rand(n_epochs * n_iters, batch_size, n_classes)
    )
    ...
    acc_metric = RunningAverage(
        Accuracy(output_transform=lambda x: [x[1], x[2]]),
        alpha=alpha,
        epoch_bound=False,
    )
    acc_metric.attach(trainer, "running_avg_accuracy")

    avg_output = RunningAverage(
        output_transform=lambda x: x[0], alpha=alpha, epoch_bound=False
    )
    avg_output.attach(trainer, "running_avg_output")
...
def _test_distrib_on_output(device):
    ...
    all_loss_values = torch.arange(
        0, k * dist.get_world_size(), dtype=torch.float64
    ).to(device)
    ...
    avg_output = RunningAverage(
        output_transform=lambda x: x, alpha=alpha, epoch_bound=False, device=device
    )
    avg_output.attach(trainer, "running_avg_output")
    ...
    def running_avg_output_update(engine):
        ...
        o = sum(
            [all_loss_values[i + j * k] for j in range(dist.get_world_size())]
        ).item()
        if engine.state.running_avg_output is None:
            engine.state.running_avg_output = o
        else:
            engine.state.running_avg_output = (
                engine.state.running_avg_output * alpha + (1.0 - alpha) * o
            )
...
def _test_distrib_on_metric(device):
    ...
    all_y_pred_batch_values = np.random.rand(
        dist.get_world_size(), n_epochs * n_iters, batch_size, n_classes
    )
