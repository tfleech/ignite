...
def test_output_as_mapping_keys_is_none():
    ...
    with pytest.raises(
        TypeError,
        match=r"Transformed engine output for DummyMetric metric should be a tuple/list",
    ):
        metric.iteration_completed(engine)
...
def test_indexing_metric():
    def _test(ignite_metric, sklearn_metic, sklearn_args, index, num_classes=5):
        ...
        assert (
            state.metrics["metric_wo_index"][index] == state.metrics["metric"]
        ).all()
    ...
    _test(
        Precision(), precision_score, {"labels": labels, "average": None}, index=labels
    )
...
def _test_distrib_sync_all_reduce_decorator(device):
    ...
    class DummyMetric(Metric):
        ...
        def reset(self):
            self.a = torch.tensor(
                [0.0, 1.0, 2.0, 3.0], device=self._device, requires_grad=False
            )
            self.a_nocomp = self.a.clone().to("cpu")
            self.b = torch.tensor(
                1.0, dtype=torch.float64, device=self._device, requires_grad=False
            )
        ...
        def compute(self):
            ...
            assert self.c == pytest.approx(
                (self.c_nocomp + 1.23456) * dist.get_world_size()
            )
