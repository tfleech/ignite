...
def main(
    ...
    RunningAverage(alpha=alpha, output_transform=lambda x: x["D_x"]).attach(
        trainer, "D_x"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--z-dim", type=int, default=100, help="size of the latent z vector"
    )
    ...
    parser.add_argument(
        "--saved-G", default="", help="path to pickled generator (to continue training)"
    )
    ...
    try:
        ...
        if (not os.path.isdir(args.output_dir)) or (
            ...
            raise FileExistsError(
                "Please provide a path to a non-existing or empty directory."
            )
