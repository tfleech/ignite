...
class ParamScheduler(metaclass=ABCMeta):
    ...
    def __init__(
        self, optimizer, param_name, save_history=False, param_group_index=None
    ):
    ...
    def load_state_dict(self, state_dict):
        ...
        if not isinstance(state_dict, Mapping):
            raise TypeError(
                "Argument state_dict should be a dictionary, but given {}".format(
                    type(state_dict)
                )
            )
...
class CyclicalScheduler(ParamScheduler):
    ...
    def __init__(
        ...
        if self.cycle_size < 2:
            raise ValueError(
                "Argument cycle_size should be positive and larger than 1, "
...
class ConcatScheduler(ParamScheduler):
    ...
    def __init__(self, schedulers, durations, save_history=False):
        ...
        if not isinstance(durations, Sequence) or not all(
            [isinstance(t, numbers.Integral) for t in durations]
        ):
            raise ValueError(
                "Argument durations should be list/tuple of integers, "
        ...
        super(ConcatScheduler, self).__init__(
            optimizer=_get_fake_optimizer(), param_name="", save_history=save_history
        )
    ...
    @classmethod
    def simulate_values(
        cls, num_events, schedulers, durations, param_names=None, **kwargs
    ):
        """Method to simulate scheduled values during num_events events.
        ...
        for i in range(num_events):
            scheduler(engine=None)
            values = [
                scheduler.optimizer_param_groups[0][param_name]
...
class LRScheduler(ParamScheduler):
    ...
    def __init__(self, lr_scheduler, save_history=False, **kwds):
        ...
        if len(lr_scheduler.optimizer.param_groups) > 1:
            raise ValueError(
                ...
                "but currently there are {} param groups".format(
                    len(lr_scheduler.optimizer.param_groups)
                )
            )
    ...
    def _replicate_lr_scheduler(lr_scheduler):
        ...
        dummy_optimizer = _get_fake_optimizer(
            optimizer_cls, **lr_scheduler.optimizer.defaults
        )
        for group in dummy_optimizer.param_groups:
            group.setdefault("initial_lr", group["lr"])
        kwargs = lr_scheduler.state_dict()
        for k in [_k for _k in kwargs.keys() if "_" == _k[0]] + [
            "base_lrs",
            "last_epoch",
        ]:
            del kwargs[k]
...
def create_lr_scheduler_with_warmup(
    ...
    if not (isinstance(warmup_duration, numbers.Integral) and warmup_duration > 1):
        raise ValueError(
            "Argument warmup_duration should be at least 2 events, but given {}".format(
                warmup_duration
            )
        )
    ...
    milestones_values = [
        ...
        (warmup_duration - 1, warmup_end_value),
    ]
    ...
    if isinstance(lr_scheduler, _LRScheduler):
        ...
        if len(init_lrs) < 1:
            raise RuntimeError(
                "Number of parameter groups of input `lr_scheduler.optimizer` is less than one."
            )
    ...
    def __init__(
        ...
        for pair in milestones_values:
            ...
            if not isinstance(pair[0], numbers.Integral):
                raise ValueError(
                    "Value of a milestone should be integer, but given {}".format(
                        type(pair[0])
                    )
                )
...
class ParamGroupScheduler:
    ...
    def __init__(self, schedulers, names):
        if not (
            ...
            and all(isinstance(scheduler, ParamScheduler) for scheduler in schedulers)
        ):
            raise ValueError(
                "Argument schedulers should be a list/tuple of parameter schedulers"
            )

        if not (
            isinstance(names, (list, tuple)) and all(isinstance(n, str) for n in names)
        ):
            raise ValueError(
                "Argument names should be a list/tuple of parameter scheduler's names"
            )

        if len(names) != len(schedulers):
            raise ValueError(
                "{} should be equal {}".format(len(schedulers), len(names))
            )
...
def _replicate_scheduler(scheduler, save_history=False):
    if isinstance(scheduler, LRScheduler):
        return LRScheduler(
            LRScheduler._replicate_lr_scheduler(scheduler.lr_scheduler),
            save_history=save_history,
        )
    elif isinstance(scheduler, ConcatScheduler):
        copy_schedulers = [
            _replicate_scheduler(s, save_history=save_history)
            for s in scheduler.schedulers
        ]
        return ConcatScheduler(
            copy_schedulers, durations=scheduler.durations, save_history=save_history
        )
