    ...
    def completed(self, engine, name):
        ...
        for i, data_by_rank in enumerate(data):
            ...
            if "fb_memory_usage" not in data_by_rank:
                warnings.warn(
                    "No GPU memory usage information available in {}".format(
                        data_by_rank
                    )
                )
                continue
        ...
        for i, data_by_rank in enumerate(data):
            ...
            if "utilization" not in data_by_rank:
                warnings.warn(
                    "No GPU utilization information available in {}".format(
                        data_by_rank
                    )
                )
                continue
            ...
            if not ("gpu_util" in util_report):
                warnings.warn(
                    "GPU utilization information does not provide 'gpu_util' information in "
                    "{}".format(util_report)
                )
                continue
