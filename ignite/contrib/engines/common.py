...
def setup_common_training_handlers(
    ...
    if dist.is_available() and dist.is_initialized():
        return _setup_common_distrib_training_handlers(
            trainer, train_sampler=train_sampler, **kwargs
        )
    else:
...
def _setup_common_training_handlers(
    ...
    if to_save is not None:
        ...
        checkpoint_handler = ModelCheckpoint(
            dirname=output_path, filename_prefix="training"
        )
        trainer.add_event_handler(
...
def setup_any_logging(
    logger, logger_module, trainer, optimizers, evaluators, log_every_iters
):
...
def setup_tb_logging(
    output_path, trainer, optimizers=None, evaluators=None, log_every_iters=100
):
...
def setup_mlflow_logging(
    trainer, optimizers=None, evaluators=None, log_every_iters=100
):
...
def save_best_model_by_val_score(
    output_path, evaluator, model, metric_name, n_saved=3, trainer=None, tag="val"
):
